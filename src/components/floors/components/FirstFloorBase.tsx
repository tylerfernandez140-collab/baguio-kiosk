...
function Model({
  ...
  const handleClick = (event: any) => {
    ...
    const isBase = (['ground', 'plane', 'stairs', 'cube', 'base', 'floor'].some(ignored => name.includes(ignored))) && !hasLabel;
    ...
    const scanMarkers = () => {
      ...
      wrapper.traverse((child: THREE.Object3D) => {
        if (child instanceof THREE.Mesh) {
          ...
          const isBase = (['ground', 'plane', 'stairs', 'cube', 'base', 'floor'].some(ignored => name.includes(ignored)) || name === 'plane001' || name === 'plane.001') && !hasLabel;
          const isPath = name.includes('path');
  ...
  return (
    <>
      ...
      {!hideLabels && officeMarkers
        .filter(office => {
          const normalizedOffice = office.name.toLowerCase().replace(/[^a-z0-9]/g, '');
          return Object.keys(labels).some(key => 
            key.toLowerCase().replace(/[^a-z0-9]/g, '') === normalizedOffice ||
            key.toLowerCase().replace(/[^a-z0-9]/g, '') === normalizedOffice.replace(/\d+$/, '')
          );
        })
        .map((office, index) => (
        <Html
